...
function EditShiftModal() {
  ...
  const hideEditShiftModal = useShiftStore(state => state.hideEditShiftModal);
  ...
  const [localUserData, setLocalUserData] = useState<formData>({
    dayOfWeek: editEmployeelModalData.dayOfWeek,
    weekNumber: editEmployeelModalData.weekNumber,
    currentYear: editEmployeelModalData.currentYear,
    employeeID: editEmployeelModalData.employeeID,
    position: editEmployeelModalData.position,
    startTime: editEmployeelModalData.startTime,
    endTime: editEmployeelModalData.endTime,
    existingShiftID: editEmployeelModalData.existingShiftID,
  })
  ...
  useEffect(() => {
    console.log(editEmployeelModalData);
  ...
  const saveData = async () => {
    try {
      ...
      const response = await axios.post('/api/business/schedule/shifts/updateshift', localUserData);
      if(response.data.status == 200){
  ...
  return (
    <div className='z-30 fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex justify-center items-center px-4 section_text'>
      ...
      <div className={`transition-opacity duration-[0.5s] ${isVisible ? 'opacity-100' : 'opacity-0'} px-6 pb-8 pt-5 w-[30rem] h-fit widget_bg border widget_border rounded-md shadow-sm`}>
        ...
        <p className='mt-2 text-sm lg:text-md'>Year: {editEmployeelModalData.currentYear}, Week: {editEmployeelModalData.weekNumber}</p>
